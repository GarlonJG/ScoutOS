...
class SemanticMemory:
    def __init__(self, memory):
        ...
        self.texts = [f"{item['user']} -> {item['assistant']}" for item in memory]
        self.ids = list(range(len(memory)))
        self.index = self.build_faiss_index(self.texts, self.model)

    def build_faiss_index(self, texts, embedder):
        start = time.perf_counter()
        if not texts:
            return None
...
def init_memory_and_index():
    ...
    logger.info(f"Embedder: {st.session_state.embedder}")
    
    semantic_memory = SemanticMemory(st.session_state.texts)
    st.session_state.faiss_index = semantic_memory.index
    st.session_state.semantic_memory = semantic_memory
...
def handle_query(user_query, use_web, search_memory, semantic_memory_enabled):
    ...
    if search_memory and semantic_memory_enabled:
        sem_mem = SemanticMemory(st.session_state.texts)
        logger.info(f"SemMemory: {sem_mem}")
        past_matches = sem_mem.search(user_query)
    elif search_memory:
        past_matches = [m for m in st.session_state.texts if any(w in m["user"].lower() for w in user_query.lower().split())]
    else:
        past_matches = []
    ...
    st.session_state.texts.append(memory_entry)
...
def main():
    ...
    if st.session_state.texts and st.session_state.texts[-1]["assistant"] == "thinking...":
        last_query = st.session_state.texts[-1]["user"]
        assistant_reply = handle_query(last_query, use_web, search_memory, semantic_toggle)
        st.session_state.texts[-1]["assistant"] = assistant_reply
        save_memory(st.session_state.texts)
        st.rerun()
